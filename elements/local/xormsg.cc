...
bool XORMsg::loop_helper(){
    ...
    //for (auto &dm: decode_map){
        ...
        //std::vector<Packet*> vp = dm.second;
        std::vector<Packet*> vp = (*it).second;
        dlock.acquire();
        ...
        if ( vp.size() == 0  ) {
            ...
            if ((Timestamp::now_steady() - vp[0]->timestamp_anno()).msec() > 100) {
                DEBUG_PRINT("removing old packets\n");
                decode_map.erase(it);
                for (auto &p : vp){
                    p->kill();
                }
