...
void XORMsg::encode(int ports, Packet *p) {
  ...
  */
  ...
  std::sort( pkt_vec.begin( ), pkt_vec.end( ), [ ]( const Packet* lhs, const
  Packet* rhs ) { return lhs->length() > rhs->length();
  });
  */

  unsigned long longest = pkt_vec[0]->length();
  // printf("pkt lengths: a: %ld, b: %ld, c: %ld\n", pkt_vec[0]->length(),
  // pkt_vec[1]->length(), pkt_vec[2]->length());

  // create an array of xor header packets for sending out
  XORProto *xorpkt_arr[3];

  // now lets create the xor packets.
  // we will create _iface variations, but lets think of that as 3 concretely
  // for now 1: pkt1 ^ pkt2 ^ pkt3 2: pkt1 ^ pkt2 (xor 1 computes pkt3) 3: pkt2
  // ^ pkt3 (xor 1 computes pkt1) [final step is take 2,3 computations xor 1 to
  // get pk2]
  ...
  for (int i = 0; i < longest / 4; i++) {
    uint32_t rng1 = rd() & 0xffffffff;
    uint32_t rng2 = rd() & 0xffffffff;
    memcpy(newB, &rng1, sizeof(uint32_t));
    memcpy(newC, &rng2, sizeof(uint32_t));
  }
  ...
  case 0:
    break;
  case 1:
    memcpy(newB, &rng1, sizeof(uint8_t));
    memcpy(newC, &rng2, sizeof(uint8_t));
    break;
  case 2:
    memcpy(newB, &rng1, sizeof(uint16_t));
    memcpy(newC, &rng2, sizeof(uint16_t));
    break;
  case 3:
    memcpy(newB, &rng1, sizeof(uint16_t) + sizeof(uint8_t));
    memcpy(newC, &rng2, sizeof(uint16_t) + sizeof(uint8_t));
    break;
  default:
    break;
  }
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    WritablePacket *data =
        Packet::make(xorpkt_arr[i], sizeof(XORProto) + header_length);
    // we done screwed up.
    if (!data)
      return;
    ...
    const click_ip *xa = reinterpret_cast<const click_ip *>(
        new_pkt->data() + sizeof(click_ether));

    std::cout << IPAddress(xa->ip_src.s_addr).s().c_str() << " -> "
              << IPAddress(xa->ip_dst.s_addr).s().c_str() << "\n";
...
void XORMsg::decode(int ports, Packet *p) {
  ...
  unsigned long dst_host = IPAddress(iph->ip_dst.s_addr);
  std::cout << IPAddress(iph->ip_src.s_addr).s().c_str() << " -> "
            << IPAddress(iph->ip_dst.s_addr).s().c_str() << "\n";

  // following from when we encoded our data and put our xor data into
  // the pkt data field, we now need to extract it
  const XORProto *xorpkt =
      reinterpret_cast<const XORProto *>(p->data() + header_length);
  ...
  printf("recv'd. flow: %lu, pkt: %lu, len: %lu, order: %u\n", flow, pktid,
         encode_length, xorpkt->Order);

  // allocate memory for the xor data to store in vector.
  unsigned char *dtemp;
  dtemp = (unsigned char *)malloc(sizeof(XORProto) + encode_length);
  memcpy(dtemp, p->data() + header_length,
         sizeof(XORProto) + encode_length); // only want the xor header and data
  ...
  */
  ...
  std::sort(pkt_vec.begin(), pkt_vec.end(),
            [](const XORProto *lhs, const XORProto *rhs) {
  ...
  const click_ip *xa =
      reinterpret_cast<const click_ip *>(a->data() + sizeof(click_ether));
  const click_ip *xb =
      reinterpret_cast<const click_ip *>(b->data() + sizeof(click_ether));
  const click_ip *xc =
      reinterpret_cast<const click_ip *>(c->data() + sizeof(click_ether));
  ...
  // std::cout << IPAddress(xa->ip_src.s_addr).s().c_str()  << " -> " <<
  // IPAddress(xa->ip_dst.s_addr).s().c_str() << "\n"; std::cout <<
  // IPAddress(xb->ip_src.s_addr).s().c_str()  << " -> " <<
  // IPAddress(xb->ip_dst.s_addr).s().c_str() << "\n"; std::cout <<
  // IPAddress(xc->ip_src.s_addr).s().c_str()  << " -> " <<
  // IPAddress(xc->ip_dst.s_addr).s().c_str() << "\n";
  ...
  std::sort(send_vec.begin(), send_vec.end(),
            [](const std::tuple<uint8_t, Packet *> lhs,
               const std::tuple<uint8_t, Packet *> rhs) {
...
// TODO bounds checking on overflow - does this matter? we will force app to
// manage staleness
int XORMsg::initialize(ErrorHandler *errh) {
  _flowid = 0; // shits and giggles we just always random this, 2**32 on
               // collision for good times
  return 0;
}
